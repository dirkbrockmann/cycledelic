...
If you just want to run the explorable you can [**Click here.**](https://raw.githack.com/dirkbrockmann/cycledelic/main/dist/index.html)
...
<html>
	<head>
		...
		<script src="https://cdn.jsdelivr.net/npm/@explorables/cycledelic/dist/index.umd.js"></script>
		...
	</head>
		...
	<body>
	...
	<script type="text/javascript">
		const explorable_instance = cycledelic.load("explorable_container")
	</script>
		...
</html>
```

The header `<script>` tag loads the bundle, the `<div>` in the document is the container in which the explorable gets anchored when the function `cycledelic("explorable_container")` gets executed at the bottom. This function needs the `<div>` container `id` as an argument. The function returns an instance of the explorable and writes it to `explorable_instance`. That variable contains functions like `halt()`, `reset()` and variables `meta` and `config`.
...
<html>
	<head>
		...
	    <script type="module">
	  	      import load from 'https://cdn.jsdelivr.net/npm/@explorables/cycledelic/dist/index.es.js';
	  	      const cycledelic = load("explorable_container");
	    </script>
	    <script nomodule src="https://cdn.jsdelivr.net/npm/@explorables/cycledelic/dist/index.umd.js"></script>	  
		...
	</head>
		...
	<body>
